...
class Company:
    ...
    def set_approval(cls, company_id, status):
        db = get_db()
        db.execute("UPDATE companies SET approval_status = ? WHERE id = ?", (status, company_id))
        db.commit()
...
class Student:
    ...
    def update(self, department=None, cgpa=None, graduation_year=None, phone=None, resume_path=None):
        db = get_db()
        db.execute(
            """UPDATE students
...
class PlacementDrive:
    ...
    def get_by_id(cls, drive_id):
        db = get_db()
        row = db.execute("SELECT * FROM placement_drives WHERE id = ?", (drive_id,)).fetchone()
        return cls(row) if row else None
    ...
    def get_all(cls):
        db = get_db()
        rows = db.execute("SELECT * FROM placement_drives ORDER BY created_at DESC").fetchall()
        return [cls(r) for r in rows]
    ...
    def set_status(cls, drive_id, status):
        db = get_db()
        db.execute("UPDATE placement_drives SET status = ? WHERE id = ?", (status, drive_id))
        db.commit()
    ...
    def applicant_count(self):
        db = get_db()
        row = db.execute("SELECT COUNT(*) as cnt FROM applications WHERE drive_id = ?", (self.id,)).fetchone()
...
class Application:
    ...
    def create(cls, student_id, drive_id):
        db = get_db()
        db.execute("INSERT INTO applications (student_id, drive_id) VALUES (?, ?)", (student_id, drive_id))
        db.commit()

    @classmethod
    def set_status(cls, app_id, status, remark=None):
        db = get_db()
        db.execute("UPDATE applications SET status = ?, remark = ? WHERE id = ?", (status, remark, app_id))
        db.commit()
